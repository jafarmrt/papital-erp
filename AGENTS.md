...
- **Version Management (v2.x Series):** Following the V10 reset, the client changelog source of truth is `src/data/changelogs/0.ts` (immutable `v1.0.0` baseline entry), `src/data/changelogs/1.ts` (`v1.x.y` series), and the ACTIVE series file `src/data/changelogs/2.ts` (`v2.x.y`). Every functional/UI/security/migration change MUST append one unique, non-duplicate `AIUpdateLog` entry to the active file (`2.ts`) and bump `package.json` `version` accordingly.
- **CHANGELOG.md Root Archive:** A condensed English archive of pre-reset history and major series milestones lives in root `CHANGELOG.md`. Historical detail lives ONLY in Git — do not re-bundle it.
...
- **V10 & Series 2.x Execution State (MUST READ at session start):** **RELEASED v2.8.7** — تمام فازهای 0..7 کامل و sign-off شده‌اند و تمامی بدهی‌های فنی رده مهم TD-033 تا TD-037 (پاکسازی و کالیبراسیون توالی‌های پایگاه‌داده، حذف کد مرده کامپوننت تست فرانت‌اند، استانداردسازی تاریخ‌های میلادی و تایپ‌های سراسری) رفع شده‌اند. مسیر ایمن و انحصاری اجرای تست‌ها `npm run test` است و اسکریپت پاکسازی داده‌های تستی از طریق `npm run db:cleanup-test` در دسترس است. Released series: `v2.x.y` (رشته‌های `package.json` + `version` در `src/app.ts` هندلر `/health` + مدخل بالای `src/data/changelogs/2.ts` همگام و روی `v2.8.7`).
...
- **Release Version Bump = ۳ محل:** همیشه هر سه با هم: (۱) `package.json`، (۲) رشته `version` در `src/app.ts` هندلر `/health`، (۳) مدخل بالای فایل چنج‌لاگ فعال (`src/data/changelogs/2.ts`).
