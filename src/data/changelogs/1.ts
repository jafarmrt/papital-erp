...
export const v1Updates: AIUpdateLog[] = [
  ...
  },
  {
    version: 'v1.9.0',
    date: '۷ شهریور ۱۴۰۵',
    title: 'تفکیک حسابداری اجزای فیش حقوقی و کسر مساعده در سند تسویه',
    summary: 'سند صدور فیش دیگر کل مبلغ را به یک حساب نمی‌زند: سهم حقوق ثابت (+پاداش) به «هزینه حقوق و دستمزد ثابت» و سهم کارکرد پرکیسی/تولیدی به «دستمزد مستقیم تولید» هزینه می‌شود و ناخالص به حقوق پرداختنی بستانکار می‌گردد. در سند تسویه، کسر مساعده/وام پرسنلی از حساب «مساعده و وام پرسنل» بستانکار و تسویه طلب پرسنلی انجام می‌شود. دو مفهوم جدید مپینگ (هزینه حقوق ثابت 6003 / سایر کسورات پرداختنی 3202) به تنظیمات حسابداری اضافه شد.',
    author: 'AI Agent (V1.9.0 — Payslip Component-wise Accounting & Advance Offset)',
    changes: [
      'سند صدور فیش حقوقی تفکیک شد: DR «هزینه حقوق ثابت» (totalFixed + پاداش) و DR «دستمزد مستقیم تولید» (کارکرد پرکیسی) / CR «حقوق پرداختنی» (ناخالص) — قبلاً کل خالص به 6002 هزینه می‌شد',
      'فیلد «کسر از مساعده/وام پرسنلی» در صدور فیش: کسر مستقیم از مانع مساعده قبلی پرسنل؛ خالص پرداختی نقدی به همان اندازه کاهش می‌یابد',
      'سند تسویه (ثبت پرداخت) بازطراحی شد: DR «حقوق پرداختنی» (ناخالص) / CR بانک (خالص نقدی) + CR «مساعده و وام پرسنل» (کسر مساعده) + CR «سایر کسورات پرداختنی» (بیمه/مالیات سهم کارمند)',
      'دو مفهوم جدید مپینگ: «هزینه حقوق و دستمزد ثابت» (پیش‌فرض 6003) و «سایر کسورات پرداختنی» (پیش‌فرض 3202) — از تنظیمات حسابداری قابل تغییر',
      'مایگریشن alt_041: ستون advance_deduction روی piecework_payrolls',
      'ردیف «کسر از مساعده و وام پرسنلی» در فیش چاپی/نمایشی',
      'اعتبارسنجی: جمع کسورات+مساعده نباید از ناخالص فیش بیشتر شود (سمت صدور و تسویه)'
    ],
    fixes: [
      'کل فیش حقوقی (شامل حقوق ثابت) که یک‌جا به حساب دستمزد تولیدی هزینه می‌شد',
      'کسر مساعده که فقط از خالص کم می‌شد ولی از حساب مساعده تسویه نمی‌شد (مانده 1301 معلق می‌ماند)'
    ]
  },
  {
    version: 'v1.8.0',
    date: '۷ شهریور ۱۴۰۵',
    title: 'بازطراحی ثبت دریافت/پرداخت وجه — پیش‌نمایش زنده سند دوبل، مفهوم مساعده پرسنل و تکمیل فرم',
    summary: 'پنل پیش‌نمایش زنده سند دوبل داخل فرم ثبت وجه (نمایش دقیق بدهکار/بستانکار با نام حساب پیش از ثبت)، مفهوم جدید «مساعده و وام پرسنل (1301)» در مپینگ با انتخاب نوع پرداخت به پرسنل، افزودن تامین‌کننده و روش چک، انتخاب طرف حساب با جستجو، هشدار شفافیت بانک بدون کدینگ، و نمایش شماره سند پس از ثبت.',
    author: 'AI Agent (V1.8.0 — Treasury Tx Modal Redesign & Employee Advance Concept)',
    changes: [
      'پنل پیش‌نمایش زنده سند دوبل در فرم ثبت دریافت/پرداخت: دو ردیف بدهکار/بستانکار با کد و نام کامل حساب معین و طرف تفصیلی — با debounce و endpoint جدید POST /accounting/treasury/preview-voucher (بدون ذخیره‌سازی)',
      'مفهوم جدید مپینگ «مساعده و وام پرسنل» (کد پیش‌فرض 1301 مطابق چارت کاربر) — پرداخت مساعده دیگر به حساب حقوق پرداختنی (3201) بدهکار نمی‌شود',
      'فیلد «نوع پرداخت به پرسنل»: تسویه حقوق (→ حقوق پرداختنی) / مساعده و وام (→ مساعده پرسنل) / سایر',
      'افزودن گزینه تامین‌کننده به انواع طرف حساب و روش «چک» به روش‌های پرداخت',
      'انتخاب مشتری/پرسنل با SearchableSelect (مناسب داده‌های پرحجم) به‌جای select ساده',
      'هشدار داخل فرم در صورت اتصال‌نبودن حساب بانکی به چارت حساب‌ها (سند صادر نخواهد شد) + هشدارهای پیش‌نمایش (عدم تطابق ارز، نبود حساب مفهوم)',
      'پیام موفقیت ثبت شامل شماره سند دوبل صادرشده',
      'منطق انتخاب طرف حساب متقابل در یک متد مشترک (resolveContraAccount) بین ثبت و پیش‌نمایش — منبع واحد حقیقت'
    ],
    fixes: [
      'پرداخت مساعده به پرسنل که اشتباهاً به حساب حقوق پرداختنی بدهکار می‌شد',
      'نامعلوم بودن سند دوبل قبل از ثبت (شفافیت کامل با پیش‌نمایش زنده)'
    ]
  },
  {
    version: 'v1.7.0',
    date: '۷ شهریور ۱۴۰۵',
    title: 'تنظیمات حسابداری — مپینگ قابل‌تنظیم سندهای اتوماتیک و همگام‌سازی شرطی کدینگ',
    summary: 'تب جدید «تنظیمات حسابداری» در تنظیمات سامانه: پیکربندی کامل مپینگ حساب‌های سندهای اتوماتیک دوبل (خزانه، چک صیادی، فروش، حقوق، تولید و بستن سال) با سوییچ روشن/خاموش هر مفهوم، و دکمه همگام‌سازی کدینگ پیش‌فرض که فقط زمانی نمایش داده می‌شود که هیچ حسابی در چارت تعریف نشده باشد.',
    author: 'AI Agent (V1.7.0 — Accounting Settings & Configurable Auto-Voucher Mappings)',
    changes: [
      'تب جدید «تنظیمات حسابداری»: لیست ۱۷ مفهوم مپینگ (دریافتنی/پرداختنی تجاری، حقوق پرداختنی، اسناد چک صیادی 1101/1102/1103/3101، فروش و تخفیف و VAT، موجودی‌ها، بهای تمام‌شده، بستن سال مالی) با توضیح کاربرد هرکدام',
      'سوییچ سفارشی‌سازی هر مفهوم: روشن = انتخاب حساب معین دلخواه از چارت؛ خاموش = بازگشت به کدینگ پیش‌فرض (ذخیره در appSettings)',
      '۴ مفهوم جدید چرخه چک (1101/1102/1103/3101) به مپینگ اضافه شد — قبلاً هاردکد بودند',
      'رفکتور کامل موتور سند اتوماتیک: چرخه چک (ثبت/ارسال به وصول/وصول/واخواست) و طرف حساب‌های متقابل تراکنش خزانه اکنون از مپینگ تنظیمات خوانده می‌شوند، نه کد ثابت',
      'دکمه «همگام‌سازی کدینگ پیش‌فرض»: فقط وقتی accountsCount صفر است نمایش داده می‌شود؛ پس از تعریف کدینگ، پیام اطلاع‌رسانی جایگزین دکمه می‌شود',
      'GET /accounting/mappings اکنون accountsCount و chartHasAccounts و disabled را هم برمی‌گرداند؛ POST فیلد disabled را ذخیره می‌کند'
    ],
    fixes: []
  },
  {
    version: 'v1.6.0',
    date: '۷ شهریور ۱۴۰۵',
    title: 'فاز ۳ خزانه — آشتی‌سنجی بانکی بیرونی، آشتی‌سنجی دفتر چک و گزارش جریان نقدی',
    summary: 'تطبیق صورت‌حساب بانک (فایل اکسل/CSV) با تراکنش‌های خزانه به‌صورت ردیف‌به‌ردیف با ثبت گروهی وضعیت آشتی‌سنجی، آشتی‌سنجی دفتر چک صیادی با حساب‌های اسناد (1101/1102/1103/3101) و کشف مغایرت، و گزارش جریان نقدی خزانه با مانده ابتدا/انتهای دوره و روند ماهانه.',
    author: 'AI Agent (V1.6.0 — Treasury Reconciliation & Cash-Flow Phase 3)',
    changes: [
      'آشتی‌سنجی بانکی: مودال اختصاصی در تب خزانه — بارگذاری فایل صورت‌حساب بانک (xlsx/csv با ستون‌های تاریخ، مبلغ، شرح، پیگیری)، تطبیق خودکار با تراکنش‌ها (تطبیق دقیق با شماره پیگیری، سپس تطبیق مبلغ)، جدول نتیجه با کیفیت تطبیق، و ثبت گروهی «آشتی‌سنجی‌شده» با Idempotency-safe',
      'مایگریشن alt_040: ستون‌های reconciled/reconciled_at/reconciled_batch روی treasury_transactions + ایندکس',
      'گزارش جریان نقدی خزانه: مانده ابتدای دوره، جمع دریافت‌ها/پرداخت‌ها و مانده پایان دوره برای هر حساب + روند ماهانه شمسی با خالص جریان؛ خروجی از تراکنش‌های واقعی خزانه (بدون ابطال‌شده‌ها)',
      'آشتی‌سنجی دفتر چک: مقایسه مانده دفتری حساب‌های اسناد (1101 نزد صندوق، 1102 در جریان وصول، 1103 واخواست، 3101 پرداختنی) با مانده موردانتظار از جدول cheques و نمایش مغایرت به تفکیک کد',
      'دکمه‌های «آشتی‌سنجی بانکی» و «جریان نقدی» در هدر تب خزانه و «آشتی‌سنجی دفتر چک» در دفتر چک صیادی'
    ],
    fixes: []
  },
  {
