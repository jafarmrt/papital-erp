...
import { 
  Info, Plus, Trash2, Search, Scale, Package, Boxes, Tag
} from 'lucide-react';
import { 
                  ...
                  return itemsSchema.map((itemSchema, iIdx) => {
                    ...
                    return (
                      <tr key={`${prod.id}_${itemSchema.id}`} className={isNeedsProcurement ? 'bg-amber-50/50' : 'hover:bg-slate-50'}>
                        {iIdx === 0 && (
                          <>
                            <td rowSpan={itemsSchema.length} className="p-2.5 font-bold text-slate-500 border-l border-slate-100 text-center align-middle bg-slate-50/50">
                              {pIdx + 1}
                            </td>
                            <td rowSpan={itemsSchema.length} className="p-2.5 font-bold text-slate-800 border-l border-slate-100 align-middle max-w-[220px] bg-slate-50/50">
                              <div className="flex items-start gap-1.5" title={`${prod.item_name || 'محصول سفارش'} ${prod.item_code ? `(${prod.item_code})` : ''}`}>
                                <div className="w-6 h-6 rounded-lg bg-amber-100 text-amber-800 flex items-center justify-center shrink-0 mt-0.5 border border-amber-300 shadow-2xs">
                                ...
                                <div className="min-w-0 flex-1">
                                  ...
                                  <div className="flex items-center gap-1 mt-0.5">
                                    <span className="inline-flex items-center gap-0.5 text-[9px] font-bold text-amber-900 bg-amber-50 px-1 py-0.2 rounded border border-amber-200">
                            ...
                            <td rowSpan={itemsSchema.length} className="p-2.5 text-center font-bold text-slate-700 border-l border-slate-100 align-middle font-mono whitespace-nowrap bg-slate-50/50">
                              <span className="px-2 py-0.5 bg-white border border-slate-200 rounded-lg text-xs">
                        ...
                        <td className="p-2.5 font-semibold text-slate-800 max-w-[220px]">
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-start gap-1.5 min-w-0 flex-1" title={effectiveName}>
                              <div className="w-5 h-5 rounded-md bg-slate-100 text-slate-600 flex items-center justify-center shrink-0 mt-0.5 border border-slate-200">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                {effectiveCode && (
                                  <span className="inline-block font-mono text-[10px] text-amber-900 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200 mt-0.5">
                                    کد: {effectiveCode}
                                  </span>
                                )}
                              </div>
