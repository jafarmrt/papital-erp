...
| TD-058 | Deployment | ممیزی Forensic BUG-18: بدون Dockerfile (مانیفست K8s به تصویر ارجاع می‌دهد) + uploads روی دیسک بدون PVC و خارج از backup → داده تصاویر در هر reschedule pod از بین می‌رود | Dockerfile (غایب), storage.ts:43-52, deploy/k8s | in_progress (v3.0.7) — آرشیو uploads در backup.sh اضافه شد؛ Dockerfile/PVC در فاز ۳ |
...
*آخرین بازبینی: v3.0.7 — اجرای فاز ۲ ممیزی جنایی V3: رفع TD-054، TD-056، TD-057، TD-060 تا TD-062 و TD-064 تا TD-066 (قیدهای یکپارچگی DB، WAC تولید، امنیت وب‌هوک/CSRF/SVG/seed عمومی/رمز، گاردهای فرانت) + پیشرفت TD-058. باقی‌مانده: TD-058/059 (فاز ۳)، TD-055/063/067 (فاز ۴). مسئولیت به‌روزرسانی: هر AI Agent / دولوپر قبل از پایان هر زیرفاز.*
